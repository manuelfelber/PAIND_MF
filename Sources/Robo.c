...
void Robo_StepForward(int steps){
	...
	for(int a=0; a<steps; a++){
		...
		for(i = 80; i < 170; i++) {
			...
			WAIT1_Waitms(5);
			Huft_R_SetPos(i); 	//Huft rechts
			Huft_L_SetPos(i); 	//Huft links
...
void walk(float steps, int T, int dir){
  ...
  //-- Feet servos also have the same offset (for tiptoe a little bit)
		   //HR, HL, FR, FL
  int A[4]= {30, 30, 33, 33}; //{30, 30, 33, 33};
  int O[4] = {0, 0, 4, -4};
...
void turn(float steps, int T, int dir){
  ...
  //--   left arc
  int A[4]= {30, 30, 30, 30}; //30 30 30 30
  int O[4] = {0, 0, 4, -4};
  double phase_diff[4] = {0, 0, DEG2RAD(-90), DEG2RAD(-90)};

  if (dir == LEFT) {
    A[0] = 20; //-- Left hip servo
  ...
  if(time>10){
    ...
    for (int iteration = 1; cntr < final_time; iteration++) {
      ...
      for (int i = 0; i < 4; i++){
		...
		switch(i){
			case 0: Huft_R_SetPos((byte)position);
			break;
			case 1: Huft_L_SetPos((byte)position);
			break;
			case 2: Fuss_R_SetPos((byte)position);
			break;
			case 3: Fuss_L_SetPos((byte)position);
			break;
		}
      }
